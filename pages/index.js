...
export default function Home() {
  ...
  const handleSearch = async (e) => {
    ...
    setLoading(true);
    // TODO: Connect to verification API
    setTimeout(() => {
      setResults({
        query,
        verified: true,
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Verify facts instantly with AI-powered source verification. Part of the Rocket+ ecosystem." />
        <meta property="og:title" content="VerifiedSXO - AI Fact Verification" />
        <meta property="og:description" content="Instant fact-checking powered by NotebookLM and trusted sources." />
            ...
            <div>
              {status === 'loading' ? (
                ...
                <button
                  ...
                  className="px-4 py-2 bg-[#0077B5] hover:bg-[#005885] text-white rounded-lg transition flex items-center gap-2"
                >
        ...
        <main className="max-w-4xl mx-auto px-4 py-16">
          <div className="text-center mb-12">
            <h1 className="text-5xl font-bold text-white mb-4">
              Verify Facts <span className="text-green-400">Instantly</span>
            </h1>
            <p className="text-xl text-gray-300">
              AI-powered fact verification using NotebookLM and trusted sources
            </p>
          </div>
          ...
          <form onSubmit={handleSearch} className="mb-12">
            <div className="flex gap-4">
              <input
                ...
                className="flex-1 px-6 py-4 bg-white/10 border border-white/20 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-400 focus:border-transparent"
              />
              <button
                type="submit"
                disabled={loading}
                className="px-8 py-4 bg-gradient-to-r from-green-400 to-emerald-500 text-white font-semibold rounded-xl hover:opacity-90 transition disabled:opacity-50"
              >
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20">
              <div className="flex items-center gap-3 mb-6">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                  results.verified ? 'bg-green-500' : 'bg-red-500'
                }`}>
                  <span className="text-2xl">{results.verified ? '✓' : '✗'}</span>
                </div>
                <div>
                  <h3 className="text-xl font-semibold text-white">
                    {results.verified ? 'Verified' : 'Unverified'}
                  </h3>
          ...
          <div className="grid grid-cols-3 gap-6 mt-12">
            {[
              { stat: '88%', label: 'Use AI' },
              { stat: '39%', label: 'See Impact' },
              { stat: '49%', label: 'The Gap We Fill' },
            ].map((item, i) => (
